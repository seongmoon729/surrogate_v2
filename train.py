...
def _train_for_object_detection(config):
    ...
    for data, step in zip(dataloader, range(start_step, end_step + 1)):
        ...
        loss_combined = w_mse * losses['mse'] + w_task * losses['d']
        ...
        if comm.is_main_process():
            ...
            writer.add_scalar('train/loss/mse', losses['mse'], step)
            writer.add_scalar('train/loss/combined', loss_combined, step)
